...
test.describe("shapes that must never reach a public repository", () => {
  ...
  test("an AWS account id, alone or inside an ARN", () => {
    expect(flagged("account 123456789012")).toBe(true);
    expect(flagged("arn:aws:iam::123456789012:role/deploy")).toBe(true);
  });

  test("a Route 53 hosted zone id", () => {
    expect(flagged("zone Z1D633PJN98FT9XYZ")).toBe(true);
  });

  test("a public IP address, but not a loopback or private one", () => {
    expect(flagged("deployed to 203.0.113.9")).toBe(true);
    expect(flagged("listening on 127.0.0.1 and 192.168.1.10")).toBe(false);
  });

  test("credentials, by shape", () => {
    expect(flagged(`token ghp_${"A".repeat(30)}`)).toBe(true);
    expect(flagged(`key AKIA${"B".repeat(16)}`)).toBe(true);
    expect(flagged(`postgres://user:npg_${"c".repeat(12)}@db/x`)).toBe(true);
    expect(flagged("-----BEGIN OPENSSH PRIVATE KEY-----")).toBe(true);
    expect(flagged("send the x-hasura-admin-secret header")).toBe(true);
  });
});
